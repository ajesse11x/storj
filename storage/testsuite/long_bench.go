...
const (
	pathDataFileName = "paths.data.gz"
...
var (
	doLongBenchmarks = flag.Bool("test-bench-long", false, "Run the long benchmark suite against eligible KeyValueStores")
...
func pathToPathDataFile() (string, error) {
	_, file, _, ok := runtime.Caller(0)
	if !ok {
		return "", errs.New("Could not get filename from runtime.Caller(); can't find data file")
	}
	testCodeDir := filepath.Dir(file)
	return filepath.Join(testCodeDir, pathDataFileName), nil
}
...
func openTestData(tb testing.TB) *KVInputIterator {
	tb.Helper()
	pathDataFile, err := pathToPathDataFile()
	if err != nil {
		tb.Fatalf("Failed to locate file with test data: %v", err)
	}
	inputIter, err := newKVInputIterator(pathDataFile)
	if err != nil {
		tb.Fatal(err)
	}
	return inputIter
...
func BenchmarkPathOperationsInLargeDb(b *testing.B, store storage.KeyValueStore) {
	...
	// create these anew with each invocation of this function; we only want these to run "once"
	// per storage backend under test.
	var initOnce sync.Once
	var cleanupOnce sync.Once

	doTest := func(name string, testFunc func(*testing.B, storage.KeyValueStore)) {
		b.Run(name, func(bb *testing.B) {
			initOnce.Do(func() { initStore(bb, store, &initialized) })
			for i := 0; i < bb.N; i++ {
				testFunc(bb, store)
			}
		})
	...
	cleanupOnce.Do(func() { cleanupStore(b, store, &initialized) })
}
...
func initStore(b *testing.B, store storage.KeyValueStore, isInitialized *bool) {
	b.Helper()
...
func cleanupStore(b *testing.B, store storage.KeyValueStore, isInitialized *bool) {
	b.Helper()
	if *isInitialized && !*noCleanDb {
		tStart := time.Now()
